...
def fetch_score_result(URL):
    ...
    for li in lis:
        ...
        if spectator =='관람객':  #베스트 관람객 내용이므로, 관람객이라면
            review = review[3:]     #잘라서 내용만 나오게 함.


        result['score'] = score
        result['review'] = review     #딕셔너리의 키 값을 그렇게 설정하고 값에 변수 넣는다.

        result_list.append({  #리스트로 만들어 붙여 넣는다.
            'score' : score,
            'review' : review
            })

    return result_list #반환
...
def fetch_reviews():
    ...
    while isLoop:
        ...
        for r in result_list:
            ...
            l_count +=1
            if l_count > count:
                isLoop= False   #리스트를 반복해서 다 읽어오면 함수 끝나도록 한다.
                break
        page +=1     #페이지를 넘어갈 때 플러스
        if not isLoop or l_count ==count:
            isLoop= False
            break

        sleepTime = random.randint(4,10)   #오류 메세지
